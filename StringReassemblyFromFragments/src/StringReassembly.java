...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        if (count == strSet.size()) {
            ...
            while (temp.size() > 0) {
                String elementN = temp.removeAny();
                if (str.indexOf(elementN) < 0) {
                    strSet.add(elementN);
                }
            }
            strSet.add(str);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            ...
            addToSetAvoidingSubstrings(result, temp);
        }
        return result;
    }
     ...
     *
     * @param args
     *            the program will prompt the user to input a file name and will
     *            read the data from the file line by line. the program then
     *            will store each line from the file into a string variable and
     *            put these string into a set call fragments. after that the
     *            program will use the method assemble to reassemble the
     *            elements in the fragments. if any string in the set is a
     *            substring of another, it will be removed from the set, and if
     *            any two string have overlapped characters(say the suffix of
     *            string1 overlap the prefix of string2) then they will be
     *            concatenated. then the program will output the reassembled
     *            element to the console screen.
     */
