...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero() && !n.isZero()) {
            if (comp > 0) {
                while (count.compareTo(m) <= 0) {
                    NaturalNumber temp = new NaturalNumber2(n);
                    NaturalNumber temp2 = new NaturalNumber2(m);
                    if (temp.divide(count).isZero()
                            && temp2.divide(count).isZero()) {
                        result.copyFrom(count);
                    }
                    count.increment();
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber substitude = new NaturalNumber2(n);
        NaturalNumber remainder = substitude.divide(TWO);
        if (remainder.isZero()) {
            result = true;
        }
        return result;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        NaturalNumber sub = new NaturalNumber2(n);
        NaturalNumber count = new NaturalNumber2(1);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean result = false;
        NaturalNumber sub = new NaturalNumber2(w);
        NaturalNumber sub2 = new NaturalNumber2(w);
        NaturalNumber TWO = new NaturalNumber2(2);
        NaturalNumber ONE = new NaturalNumber2(1);
        ...
        powerMod(sub, TWO, n);
        if (sub.compareTo(ONE) == 0) {
            result = true;
        }
        powerMod(sub2, power, n);
        if (sub2.compareTo(ONE) != 0) {
            result = true;
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            while (i <= 10) {
                ...
                NaturalNumber witCan = new NaturalNumber2(randomNumber(n));
                if (witCan.compareTo(TWO) < 0) {
                    witCan.add(TWO);
                    if (!isWitnessToCompositeness(witCan, n)) {
                        count++;
                    }
                    i++;
                } else if (witCan.compareTo(substitude) > 0) {
                    witCan.subtract(TWO);
                    if (!isWitnessToCompositeness(witCan, n)) {
                        count++;
                    }
